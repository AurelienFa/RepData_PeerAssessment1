...
- The dataset contains `r na` NAs among `r nobs` observations, so `r pct`% overall.  
- The observations range from `r fday` to `r mday` and contain `r nbdays` days of data.
- Those number of days should account for `r nobs.should.formated` 5' interval observations, whereas we have here `r nobs` so we have `r comp`% of the whole data that could have been collected (except for NAs).
...
`r formatC(s_mean, format = "d")` whereas the median is `r s_median`. So both are very close to each other.
...
As already mentionned in the above section, we have `r na` NAs among our data, which corresponds to `r pct`% overall of data points with NA.
